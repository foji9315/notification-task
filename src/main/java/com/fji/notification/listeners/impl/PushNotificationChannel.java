...
public class PushNotificationChannel implements ChannelListener {
    ...
    public NotificationLog sentNotification(NotificationMessage message) {
        NotificationLog notificationLog = NotificationLog.builder()
                ...
                .build();
        notificationLog.setChannelType(ChannelEnum.PUSH);
        try {
            log.info("Push to {}: Someone has publish a message with category {}, where body is {}", userData.getName(), message.getCategory(), message.getMessage());
            ...
            log.error("Error while sending PUSH to {} for messageId : {}. NotificationLog created for retry later", userData.getEmail(), message.getId());
        }
