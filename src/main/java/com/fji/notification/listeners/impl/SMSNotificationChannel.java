...
@Slf4j
@AllArgsConstructor
public class SMSNotificationChannel implements ChannelListener {
    ...
    public NotificationLog sentNotification(NotificationMessage message) {
        NotificationLog notificationLog = NotificationLog.builder()
                ...
                .build();
        notificationLog.setChannelType(ChannelEnum.SMS);
        try {
        log.info("SMS to {}: Someone has publish a message with category {}, where body is {}", userData.getPhoneNumber(), message.getCategory(), message.getMessage());
            ...
            log.error("Error while sending SMS to {} for messageId : {}. NotificationLog created for retry later", userData.getPhoneNumber(), message.getId());
        }
